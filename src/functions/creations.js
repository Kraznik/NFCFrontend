...
export const uploadFile = async (file, AccessToken, momentsData) => {
  ...
  try {
    ...
    await ClaimOneForCreator(nftTypeId, collabAddress);
...
const getLazyMintSignature = async (values, creatorTypeId) => {
  ...
  try {
    ...
    try {
      // const url = `https://honetpot.kraznikunderverse.com/getLazyMintSignature/ethcc`;
      const url = `${config.apiBaseUrl}/getLazyMintSignature`;
      const post_data = {
        hash,
      };
...
const ClaimOneForCreator = async (nftTypeId, collabAddress) => {
  console.log("nft type id: ", nftTypeId);
  console.log("collab wallet: ", collabAddress);
  console.log("add claim to backend...");
  await Claim(nftTypeId, collabAddress);
};

const Claim = async (nftTypeId, collabAddress) => {
  // console.log("nft type id: ", nftTypeId);
